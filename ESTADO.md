...
## 33. Cadastro por pedido de acesso + resumo da pauta + pasta final (17/09, tarde)

- **Pedido do Bruno:** "ele acessa, gera uma solicitação para mim, eu aceito e determino o perfil". Antes: o gestor criava a conta no Console do Firebase, copiava o UID e colava no cadastro.
- **Como ficou:** `login.html` ganhou "Primeiro acesso? Pedir acesso ao gestor" (nome · e-mail · senha de 8+ · setor ou função). Cria a conta no Authentication e grava `solicitacoes_acesso/{uid}` com `status:'pendente'`; sai em seguida. A conta NÃO entra no sistema (sem doc em `/usuarios` o guard nega, com mensagem nova). **Gestor:** Hoje mostra "N pedido(s) de acesso esperam você"; em Usuários, bloco "Pedidos de acesso" → **Liberar acesso** abre o cadastro já preenchido (UID travado, nome, e-mail) e o gestor só escolhe perfil e setor; **Recusar** marca o pedido.
- **Regra nova (o Bruno publica):** `match /solicitacoes_acesso/{uid}` — create só pelo próprio uid, com e-mail igual ao do token, `status == 'pendente'`, só 6 campos e tamanhos limitados; get pelo dono ou gestor; list/update só gestor; delete nunca. Arquivo completo e passo a passo em `2026-09-17/REGRAS_FIRESTORE/`. Até publicar, o pedido falha com aviso claro e as leituras do gestor voltam vazias (sem erro na tela).
- **Reuniões:** campo "O que será tratado (resumo)" no bloco Abertura (`pautaTexto`), que entra no topo da pauta copiada.
- **Testado no simulador:** pedido ponta a ponta (validação, doc com os 6 campos, id = uid), bloco de pedidos e editor preenchido, faixa do gestor, resumo da pauta gravando. Não testado contra o Firebase real: criação de conta e a regra nova (dependem da publicação).
- **Pasta final:** `Sistema 5W2H_atividades GTfsa/2026-09-17/` com SISTEMA_FINAL (tudo o que está no ar), REGRAS_FIRESTORE, ENTREGAS (PDF e PowerPoint) e ESTADO.md.
