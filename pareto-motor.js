...
(function (root) {
  ...
  /* O texto do "i" é contrato: tem de dizer a MESMA conta que o motor faz.
     Correção de 16/09 (auditoria, #41): a fórmula dizia "atrasos + travas +
     puladas" desde a especificação original, mas o motor também conta a
     entrega fora do prazo (ver motivoFalha). Com uma rotina que sempre
     entrega três dias tarde, o motor dava 2 falhas e o texto somava 0 — quem
     conferisse a conta pelo "i" acharia que o Pareto está errado, quando o
     errado era o texto. Os dados também não diziam que a conta para em
     hoje: a ocorrência de hoje entra nas previstas; a de amanhã, não. */
  function formula() {
    return {
      titulo: 'Como esta lista é montada',
      linhas: [
        ['Fórmula', 'Falhas por atividade desde o marco zero = atrasos + entregas fora do prazo + travas + puladas, contando uma vez por ocorrência. Taxa = falhas ÷ previstas. A lista vai até 80% do acumulado, com no mínimo 3 e no máximo 10 linhas.'],
        ['Dados', 'Ocorrências planejadas a partir de ' + fmtCurto(MARCO_ZERO) + ' com data (a reprogramada, quando houver) até hoje, cruzadas com a atividade. A de hoje já entra nas previstas; a reprogramada para depois de hoje fica de fora. Nada é gravado: o número é calculado na hora.'],
